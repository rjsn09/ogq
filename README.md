...
기존 로컬에서 서버를 돌리는 방식은 컴퓨터를 항상 켜둘수도 없고, 하드웨어의 한계로 사진 한 장을 생성하는데 20분씩 걸리는 문제가 있었다.
-> 이를 해결하기 위해 runpod에서 a40 gpu를 대여하여 그곳에서 서버를 돌려 cpu_offload로 인한 병목현상을 제거하였다.

---

기존에 서버에 저장되어있는 테마 프롬프트는 유저가 원하는 테마를 완전히 충족시켜줄 수 없다. 
-> 이를 해결하기 위하여 각 테마(셀)마다 유저가 선택적으로 프롬프트를 작성할 수 있게 변경하였다. 이를 입력하면 기존 서버의 테마 프롬프트와 같이 사용하되, RANK를 유저의 프롬프트가 더 높게 하여 유저의 프롬프트가 더 적극적으로 반영되도록 구현하였다.

---

기존의 프롬프트는 기준이미지와 테마 프롬프트 모두에서 자세, 구도, 외형 특징 구분 없이 모두 RANK에 따라 중요도를 매겨 최종 프롬프트로 변형되었다. 이로 인해 모든 이미지들의 구도가 기준이미지를 따라가는 경향이 생겼다
-> 이를 해결하기 위하여 유저 프롬프트와 테마 프롬프트에서 구도, 표정 등의 정보를 참조하고(중요도: 유저 프롬프트 > 테마 프롬프트) 외형 특정등의 정보는 기준 이미지

---

기존의 시스템에서는 사용자에게 개인정보 처리방침 등에 동의하게 하지 않는 등 저작권 등의 문제에 휘말릴 우려가 있었다.
-> 서비스 이용약관 및 개인정보 처리방침 동의를 넣어 사용자가 동의해야 이모티콘 생성할 수 있도록 하였다.

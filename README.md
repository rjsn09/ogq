...
기준 이미지를 image2image 생성의 입력으로 사용하더라도 strength값을 높이면 캐릭터의 자세를 크게 변경할 수 있지만, 동시에 얼굴이나 의상 등 캐릭터의 고유 특징까지 변형되는 문제 발생

-> OpenPose 기반 ControlNet을 추가하여 캐리거의 외형과 새로운 자세를 별도의 입력으로 제어하도록 하였다.

---

기존에는 감정별 프롬프트에 "웃눈 표정", "화난 표정", "손을 흔드는 모습"과 같은 단어 설명만 전달하였다. 그러나 이만으로는 기준 이미지의 자세를 그대로 유지하거나 원하는 동작이 제대로 만들어지지 않는 문제 발생

->  llm을 이용하여 프롬프트를 구체화하여 보다 원하는 동작을 직접적으로 제어할 수 있도록 하였다.

---

기존에는 사용자가 업로드한 원본 이미지를 그대로 참조이미지로 사용해 이모티콘을 생성하였는데, 이 때문에 원본 이미지의 그림체, 자세, 구도 등의 영향이 모든 이미지에 적용되는 문제가 발생하였다.

-> 사용자가 업로드한 원본 이미지로부터 하나의 기준 이미지(이모티콘 스타일)를 생성하고, 사용자에게 승인받는 방법으로 기준이미지가 제대로 생성되지 않았을 경우(퀄리티가 너무 낮거나 특징이 사라진 경우)모든 이모티콘이 망가지는 경우를 방지하였다.
...
원본 이미지
-> WD14 Tagger를 이용한 캐릭터 프로필 생성
-> IP-Adapter + 캐릭터 프로필 + LoRA를 이용한 기준 이미지 생성
-> 생성된 기준 이미지를 WD14로 다시 분석하여 기준이미지 프로필 생성
-> 사용자가 기준이미지를 확인하고 승인/재생성
-> LLM이 감정별 상세 프롬프트와 OpenPose 관절 좌표 생성
-> 기준 이미지를 img2img의 초기 이미지로 사용
-> OpenPose ControlNet으로 새로운 자세 적용
-> LoRA를 이용하여 이모티콘 스타일 유지
-> 여러 후보이미지를 생성
-> CLIP으로 기준이미지 및 프롬프트와의 유사도 평가 후 가장 높은 점수의 이미지 선택
-> 배경 제거
-> 후처리 후 최종 이모티콘 세트 출력
